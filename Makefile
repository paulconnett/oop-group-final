...
DOCS = game/docs
FILES = game/game.py game/room.py game/mob.py game/player.py game/enemy.py game/item.py

all: check-style check-types run-test run-test-coverage create-docs create-uml clean
	@echo "Make all done...!"
...
ifeq ($(shell which pytest), ) # if pytest not found
	...
	pytest --verbose --color=yes game/tests/
	@echo "All unittests passed!"

.PHONY: run-test-coverage
run-test-coverage: create-doc-folder
	pytest --verbose --color=yes --cov=. --cov-report=term-missing --cov-report=html:$(DOCS)/htmlcov game/tests/
	@echo "All unittests passed!"
...
create-docs: create-doc-folder
	pdoc --output-dir $(DOCS)/code-docs $(FILES)
